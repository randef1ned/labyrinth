...
#'
random_graph <- function(n_element = sample(10:500, 1),
                         float = as.logical(sample(0:1, 1)),
                         sparse = as.logical(sample(0:1, 1))) {
  while (TRUE) {
  ...
  if (float && sparse) {
    ...
    graph[graph == 1] <- runif(length(graph[graph == 1]), min = 0.01, max = 3.0)
  }
  return(graph)
}
